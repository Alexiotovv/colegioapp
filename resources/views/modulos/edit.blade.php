        ...
        <form method="POST" action="{{ route('admin.modulos.update', $modulo) }}" id="moduloForm">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('ruta') is-invalid @enderror" 
                        ...
                        @foreach($rutasDisponibles as $rutaItem)
                            <option value="{{ $rutaItem['name'] }}" {{ old('ruta', $modulo->ruta) == $rutaItem['name'] ? 'selected' : '' }}>
                                {{ $rutaItem['name'] }} → {{ $rutaItem['uri'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div id="iconoSeleccionado" class="mt-2" style="{{ old('icono', $modulo->icono) ? 'display: block;' : 'display: none;' }}">
                        <span class="badge bg-secondary">
                            <i class="fas {{ old('icono', $modulo->icono) }}" id="iconoSeleccionadoPreview"></i>
                            <span id="iconoSeleccionadoTexto">{{ old('icono', $modulo->icono) }}</span>
                        </span>
                    </div>
                    @error('icono')
                        <div class="invalid-feedback">{{ $message }}</div>
...
<script>
    $(document).ready(function() {
        ...
        const iconosDisponibles = [
            ...
            'fa-door-open', 'fa-calendar-week', 'fa-calendar-alt', 'fa-address-card',
            'fa-print', 'fa-tags', 'fa-tag', 'fa-plus', 'fa-plus-circle', 'fa-minus',
            'fa-trash', 'fa-trash-alt', 'fa-undo', 'fa-redo', 'fa-search', 'fa-filter',
            'fa-sort', 'fa-sort-down', 'fa-sort-up', 'fa-download', 'fa-upload',
            'fa-external-link-alt', 'fa-link', 'fa-paperclip', 'fa-image', 'fa-file',
            'fa-file-pdf', 'fa-file-excel', 'fa-file-word', 'fa-envelope', 'fa-phone',
            'fa-mobile-alt', 'fa-map-marker-alt', 'fa-building', 'fa-globe', 'fa-language',
            'fa-palette', 'fa-paint-brush', 'fa-code', 'fa-terminal'
        ];
