{{-- resources/views/avance-libretas/index.blade.php --}}
@extends('layouts.app')

@section('title', 'Exportar Avance Libretas')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4>
            ...
            Exportar Avance Libretas
        </h4>
        <div class="d-flex gap-2">
            @if(auth()->user()->puedeAccederModulo('avance-exportar-por-alumno'))
                <a href="{{ route('admin.avance-libretas.exportar-por-alumno.index') }}" class="btn btn-outline-primary">
                    <i class="fas fa-user-graduate me-2"></i> Exportar Avance Libreta por Alumno
                </a>
            @endif
        </div>
    </div>
...
$(document).ready(function() {
    ...
    function cargarAlumnosAutomaticamente(aulaId, periodoId) {
        ...
        $.ajax({
            url: '{{ route("admin.avance-libretas.alumnos-by-aula") }}',
            method: 'GET',
            data: { aula_id: aulaId },
    ...
    $('#btnExportarAula').on('click', function() {
        ...
        let form = $('<form>', {
            'method': 'POST',
            'action': '{{ route("admin.avance-libretas.exportar-aula") }}'
        }).append($('<input>', {
            'name': 'aula_id',
            'value': aulaId,
            'type': 'hidden'
    ...
    $('#btnPrevisualizarAula').on('click', function() {
        ...
        let url = '/admin/avance-libretas/previsualizar-aula?aula_id=' + aulaId + '&periodo_id=' + periodoId;
...
function previsualizar(matriculaId) {
    let periodoId = $('#periodo_id').val();
    window.open('/admin/avance-libretas/previsualizar?matricula_id=' + matriculaId + '&periodo_id=' + periodoId, '_blank');
}
...
function exportarAlumno(matriculaId) {
    ...
    let form = $('<form>', {
        'method': 'POST',
        'action': '{{ route("admin.avance-libretas.exportar-alumno") }}'
    }).append($('<input>', {
        'name': 'matricula_id',
        'value': matriculaId,
        'type': 'hidden'
