...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
            <a href="{{ route('admin.aulas.show', $aula) }}" class="btn btn-info">
        ...
        <form method="POST" action="{{ route('admin.aulas.update', $aula) }}">
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ $aula->codigo }}" readonly disabled>
                    <small class="text-muted">El código es automático y no se puede modificar</small>
                </div>
                
                <div class="col-md-4 mb-3">
                    <label for="nombre" class="form-label required-field">Nombre del Aula</label>
                    <input type="text" class="form-control @error('nombre') is-invalid @enderror" 
                           id="nombre" name="nombre" value="{{ old('nombre', $aula->nombre) }}" required>
                    @error('nombre')
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('turno') is-invalid @enderror" id="turno" name="turno" required>
                        ...
                        @foreach($turnos as $key => $label)
                            <option value="{{ $key }}" {{ old('turno', $aula->turno) == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('nivel_id') is-invalid @enderror" id="nivel_id" name="nivel_id" required>
                        ...
                        @foreach($niveles as $nivel)
                            <option value="{{ $nivel->id }}" {{ old('nivel_id', $aula->nivel_id) == $nivel->id ? 'selected' : '' }}>
                                {{ $nivel->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('grado_id') is-invalid @enderror" id="grado_id" name="grado_id" required>
                        ...
                        @foreach($grados as $grado)
                            <option value="{{ $grado->id }}" {{ old('grado_id', $aula->grado_id) == $grado->id ? 'selected' : '' }}>
                                {{ $grado->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select @error('seccion_id') is-invalid @enderror" id="seccion_id" name="seccion_id" required>
                        ...
                        @foreach($secciones as $seccion)
                            <option value="{{ $seccion->id }}" {{ old('seccion_id', $aula->seccion_id) == $seccion->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('anio_academico_id') is-invalid @enderror" id="anio_academico_id" name="anio_academico_id" required>
                        ...
                        @foreach($anios as $anio)
                            <option value="{{ $anio->id }}" {{ old('anio_academico_id', $aula->anio_academico_id) == $anio->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select select2 @error('docente_id') is-invalid @enderror" id="docente_id" name="docente_id">
                        ...
                        @foreach($docentes as $docente)
                            <option value="{{ $docente->id }}" {{ old('docente_id', $aula->docente_id) == $docente->id ? 'selected' : '' }}>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" class="form-control @error('capacidad') is-invalid @enderror" 
                           id="capacidad" name="capacidad" value="{{ old('capacidad', $aula->capacidad) }}" min="1" max="100">
                    @error('capacidad')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('descripcion') is-invalid @enderror" 
                          id="descripcion" name="descripcion" rows="3">{{ old('descripcion', $aula->descripcion) }}</textarea>
                @error('descripcion')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="activo" id="activo" value="1" {{ old('activo', $aula->activo) ? 'checked' : '' }}>
...
$(document).ready(function() {
    ...
    let gradosOriginales = $('#grado_id').html();
                ...
                success: function(response) {
                    ...
                    if (response.length > 0) {
                        for (let grado of response) {
                            let selected = (grado.id == {{ $aula->grado_id }}) ? 'selected' : '';
