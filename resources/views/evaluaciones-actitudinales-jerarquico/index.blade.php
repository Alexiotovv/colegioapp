{{-- resources/views/evaluaciones-actitudinales-jerarquico/index.blade.php --}}
@extends('layouts.app')

@section('title', 'Gestión de Evaluaciones Actitudinales')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4>
            ...
            Gestión de Evaluaciones Actitudinales
        </h4>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalEvaluacion" onclick="resetEvaluacionForm()">
            <i class="fas fa-plus me-2"></i> Nueva Evaluación Actitudinal
        </button>
    </div>
    
    <div class="tree-container">
        <div id="treeContent">
            @include('evaluaciones-actitudinales-jerarquico.partials.tree', ['niveles' => $data])
        </div>
    </div>
</div>

@include('evaluaciones-actitudinales-jerarquico.modals.evaluacion-modal')
@endsection
...
$(document).ready(function() {
    ...
    window.showEvaluacionModal = function(evaluacionId, nivelId) {
        ...
        if (evaluacionId) {
            $('#modalEvaluacionTitle').text('Editar Evaluación Actitudinal');
            $('#btnSaveEvaluacion').html('<i class="fas fa-save me-2"></i> Actualizar');
            
            $.ajax({
                url: '/admin/evaluaciones-actitudinales-jerarquico/evaluacion/' + evaluacionId,
                method: 'GET',
            ...
            $('#modalEvaluacionTitle').text('Nueva Evaluación Actitudinal');
    ...
    window.toggleEvaluacion = function(evaluacionId) {
        $.ajax({
            url: '/admin/evaluaciones-actitudinales-jerarquico/evaluacion/' + evaluacionId + '/toggle',
            method: 'PATCH',
            data: { _token: '{{ csrf_token() }}' },
            success: function(response) {
                if (response.success) {
    ...
    $('#formEvaluacion').on('submit', function(e) {
        ...
        let url = evaluacionId ? '/admin/evaluaciones-actitudinales-jerarquico/evaluacion/' + evaluacionId : '{{ route("admin.evaluaciones-actitudinales-jerarquico.store") }}';
