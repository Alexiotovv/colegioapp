...
@section('title', 'Gestión de Tipos de Inasistencia')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4>
            ...
            Gestión de Tipos de Inasistencia
        </h4>
        <div>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalTipoInasistencia" onclick="resetTipoForm()">
    ...
    <div class="tree-container" id="treeContainer">
        <div id="treeContent">
            @include('tipos-inasistencia-jerarquico.partials.tree', ['niveles' => $data])
        </div>
    </div>
</div>

<!-- Modal Tipo Inasistencia -->
@include('tipos-inasistencia-jerarquico.modals.tipo-modal')
@endsection
...
$(document).ready(function() {
    ...
    window.showTipoModal = function(tipoId, nivelId) {
        ...
        if (tipoId) {
            $('#modalTipoTitle').text('Editar Tipo de Inasistencia');
            $('#btnSaveTipo').html('<i class="fas fa-save me-2"></i> Actualizar');
            
            $.ajax({
                url: '/admin/tipos-inasistencia-jerarquico/tipo/' + tipoId,
                method: 'GET',
            ...
            $('#modalTipoTitle').text('Nuevo Tipo de Inasistencia');
        ...
        $('#modalTipoInasistencia').modal('show');
    };
    ...
    window.toggleTipo = function(tipoId) {
        $.ajax({
            url: '/admin/tipos-inasistencia-jerarquico/tipo/' + tipoId + '/toggle',
            method: 'PATCH',
            data: { _token: '{{ csrf_token() }}' },
            success: function(response) {
                if (response.success) {
    ...
    $('#formTipo').on('submit', function(e) {
        ...
        let url = tipoId ? '/admin/tipos-inasistencia-jerarquico/tipo/' + tipoId : '{{ route("admin.tipos-inasistencia-jerarquico.store") }}';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    $('#modalTipoInasistencia').modal('hide');
