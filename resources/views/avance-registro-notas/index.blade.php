...
<style>
    .filter-card {
        ...
        /* Ajuste suave de tamaño para los filtros principales */
        #aula_id,
        #curso_id,
        #periodo_id {
            font-size: 0.88rem;
...
$(document).ready(function() {
                ...
                bodyHtml += `
                    <td style="text-align: center; vertical-align: middle;">
                        <div class="select-wrapper" style="display:inline-block; position:relative;">
                            <input type="text"
                                   class="form-control form-control-sm nota-select nota-input ${notaGuardada}"
                                   data-matricula="${matricula.id}"
                                   data-competencia="${competencia.id}"
                                   data-nota-id="${notaId}"
                                data-original-value="${notaValue || ''}"
                                   data-prev-value="${notaValue || ''}"
                                   value="${notaValue || ''}"
                                   maxlength="6"
                                   autocomplete="off"
    ...
    function configurarEventosTabla() {
        ...
        function opcionesNotasNormalizadas() {
            return (opcionesNotas || []).map(function(op) {
                return normalizarNota(op);
            });
        }

        function esNotaPermitidaExacta(valor) {
            let normalizada = normalizarNota(valor);
            if (!normalizada) return false;
            return opcionesNotasNormalizadas().includes(normalizada);
        }

        function esPrefijoPermitido(valor) {
            let normalizada = normalizarNota(valor);
            if (normalizada === '') return true;
            return opcionesNotasNormalizadas().some(function(op) {
                return op.startsWith(normalizada);
            });
        }

        function enfocarSiguienteInputNota(inputActual) {
            let $inputs = $('.nota-input:enabled');
        ...
        function actualizarEstadoNotaDesdeInput($input, valor, esExacta) {
            let $hiddenInput = $input.closest('td').find('.nota-valor');
            let $btnMensaje = $input.closest('td').find('.btn-message');
            let originalValue = normalizarNota($input.attr('data-original-value') || '');
            let valorNormalizado = normalizarNota(valor);
            let cambioReal = esExacta && valorNormalizado !== originalValue;
            ...
            if (esExacta) {
                ...
                $input.addClass('nota-guardada');
                $input.attr('data-prev-value', valor);
                $input.data('prev-value', valor);
            } else {
                $hiddenInput.val('');
                $btnMensaje.data('nota', '');
                $input.removeClass('nota-guardada');
            }
            ...
            if (requiereConclusionAhora && cambioReal) {
                $btnMensaje.find('i').css('color', '#dc3545');
            } else {
                if ((ruleActivePrimaria && isPrimaria && !['B', 'C'].includes(valor)) ||
                    (ruleActiveSecundaria && isSecundaria && valor !== 'C')) {
                    let pendingKey = `${$input.data('matricula')}_${$input.data('competencia')}`;
                    delete conclusionesPendientes[pendingKey];
                }
                // Mantener color si no hay cambio real para no marcar en blur sin edición.
                if (cambioReal || !esExacta) {
        ...
        // Entrada de nota como texto: solo valores permitidos y navegación con Enter/Tab
        $('.nota-input').off('input').on('input', function() {
            let $input = $(this);
            let valor = normalizarNota($input.val());

            if (!esPrefijoPermitido(valor)) {
                let previo = normalizarNota($input.data('prev-value') || '');
                $input.val(previo);
                valor = previo;
            }

            actualizarEstadoNotaDesdeInput($input, valor, esNotaPermitidaExacta(valor));
        });

        $('.nota-input').off('blur').on('blur', function() {
            let $input = $(this);
            let valor = normalizarNota($input.val());

            if (!esNotaPermitidaExacta(valor)) {
                valor = '';
            }
        ...
        $('.nota-input').off('focus').on('focus', function() {
            this.select();
        });

        $('.nota-input').off('click').on('click', function() {
            this.select();
        });

        $('.nota-input').off('keydown').on('keydown', function(e) {
            if (e.key === 'Enter' || e.key === 'Tab') {
                e.preventDefault();
                enfocarSiguienteInputNota(this);
            }
        });
    }
